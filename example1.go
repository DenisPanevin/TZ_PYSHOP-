...
func getScore(scores *[]ScoreStamp, offset int) (Score, error) {
    if offset < 0 || offset >= len(*scores) {
        return Score{}, fmt.Errorf("offset %d is out of range", offset)
    }

    return (*scores)[offset].Score, nil
}
